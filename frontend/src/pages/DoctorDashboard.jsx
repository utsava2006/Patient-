...
export default function DoctorDashboard() {
  ...
  const handleUpdateSettings = async (e) => {
    ...
    try {
      const res = await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/doctor/${doctor.id}/credentials`, {
  ...
  useEffect(() => {
    ...
    const fetchStatsAndPatients = async () => {
      try {
        const statsRes = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/doctor/stats`);
        setStats(statsRes.data);
        const patientsRes = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/doctor/patients`);
  ...
  const handleSearch = async (e, codeOverride) => {
    ...
    setPatient(null);
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/patient/${codeToSearch}`);
      setPatient(res.data);
  ...
  const handlePrescribe = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/doctor/prescribe`, {
  ...
  const handleScheduleVisit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/doctor/patient/${patient.patientCode}/visit`, {
        nextVisitDate
      });
  ...
  const handleDeleteVisit = async (visitId) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/doctor/visit/${visitId}`);
      handleSearch({ preventDefault: () => {} });
    } catch (err) {
      console.error(err);
  ...
  const handleDeletePrescription = async (prescriptionId) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/doctor/prescription/${prescriptionId}`);
      handleSearch({ preventDefault: () => {} });
    } catch (err) {
      console.error(err);
  ...
  const handleLogVisit = async () => {
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/doctor/log-visit`, {
        patientId: patient.id,
        doctorId: doctor.id
      });
  ...
  const handleDeletePatient = async (e, patientId, patientName) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/doctor/patient/${patientId}`);
